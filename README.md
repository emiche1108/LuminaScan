...
LuminaScan は、高度な画像処理技術を用いて肌の状態を数値化・可視化する肌診断アプリです。<br>
...
特に「データをもとに未来を予測し、業務の最適化に貢献できる」需要予測の面白さに魅了され<br>
「より深くデータを分析し、活用できるようになりたい」という想いが芽生えました。<br>

アプリケーション開発に際して<br>
需要予測の分野に興味がありましたが、個人での開発や検証が難しく、より身近なテーマ でデータ活用の実践ができないかと考えた結果
「肌の状態を画像解析技術を用いて数値化し、客観的に分析する」ことに着目し、LUMINA Scan の開発に至りました。

具体的には、OpenCV を活用した エッジ検出、GLCM（グレーレベル共起行列）、ガウシアン差分、ガボールフィルタ などの手法を組み合わせ、
肌のシワ・シミ・くすみなどを評価します。
機械学習モデルはまだ導入していませんが、今後、AI（ディープラーニング）を活用し、より高度な肌診断を実現することを目指しています。

本プロジェクトを通じて、画像解析の応用とデータ分析の知見を深める だけでなく、将来的な AI活用の可能性 を探求していきます。



AIを使えば、肌の状態を数値化し、客観的に分析できるのではないか？<br>
そう考え、データ分析の理解を深めるとともに、プログラマーとしての適性を試す 個人プロジェクトとして LUMINA SKIN を開発しました。<br>
<br>
アプリケーション制作を通じて、AIの実践的な活用方法を学び、機械学習モデルの本質を探求し、より深い知識とスキルを身につけること を目指しています。<br>
<br>
<br>
<br>
